            ...
            <div class="bg-primary bg-opacity-70 text-left p-3 text-gray-50 group-hover:bg-opacity-100 transition duration-500 ">
              ...
              <div class="flex text-xs italic gap-1 items-center overflow-x-scroll no-scrollbar  focus:touch-pan-x">
                <template v-for="(des, index) in packages.paquetes_destinos">
                  {{des.destinos.nombre}}
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor" class="w-3 h-3 " v-if="index < packages.paquetes_destinos.length - 1">
...
<!--          <button v-show="packages.codigo_f" @click="pack(packages.codigo_f)" class="wtrvl-checkout_button btn-ternary mb-2 block w-full" id="wetravel_button_widget">Book Now</button>-->
  ...
  <section class="container">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <nuxt-link :to="'/peru-travel-packages/'+packages.url" class="bg-white grid grid-cols-1 content-between overflow-hidden rounded-md border group cursor-pointer hover:shadow-lg transition duration-500 ease-in-out" v-for="(packages, index) in listPackages" :key="index">
        <div class="">
          <div class="overflow-hidden relative">
          ...
          <div class="p-6">
            ...
            <h2 class="text-xl mb-4 font-playfair-display tracking-wide">{{ packages.titulo }}</h2>
